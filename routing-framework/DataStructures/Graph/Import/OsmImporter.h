...
class OsmImporter {
 public:
  ...
  void init(const std::string& filename) {
    ...
    mapping = RoutingKit::load_osm_id_mapping_from_pbf(filename + ".osm.pbf", nullptr, isWayOpenForVehicles);
    wayIdMapper = RoutingKit::IDMapper(mapping.is_routing_way);
    const auto numWaysOpenForVehicles = mapping.is_routing_way.population_count();
